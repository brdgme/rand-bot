...
fn spec_entry(specs: &command::Specs, spec: &command::Spec, rng: &mut ThreadRng) -> Vec<String> {
    match spec.kind {
        command::Kind::Int { min, max } => {
            vec![format!("{}", bounded_i32(rng.gen(), min.unwrap_or(i32::MIN), max.unwrap_or(i32::MAX)))]
        }
        ...
        command::Kind::Chain(ref chain) => chain.iter().flat_map(|c| spec_entry(specs, c, rng)).collect(),
    }
}
...
impl<T: Gamer> Botter<T> for RandBot {
    fn commands(_player: usize,
        ...
        spec_entry(&command_spec, &command_spec.specs.get(&command_spec.entry).unwrap(), &mut rng);
        vec![]
    }
}
